...
func (s *Store) ListManagedUsersWithAccess(ctx context.Context, page, pageSize int, hubID int64, search string, access AccessFilter, sort Sort) ([]ManagedUser, Page, error) {
	...
	if err := s.Pool.QueryRow(ctx, `SELECT count(*) FROM managed_users u LEFT JOIN users account ON lower(account.username)=lower(u.username) WHERE ($1=0 OR u.hub_id=$1) AND ($2='' OR u.username ILIKE $3 ESCAPE '\') AND `+countAccessSQL, countArgs...).Scan(&total); err != nil {
		return nil, Page{}, err
	}
	...
	rows, err := s.Pool.Query(ctx, `
		...
		WHERE ($1=0 OR u.hub_id=$1) AND ($2='' OR u.username ILIKE $3 ESCAPE '\') AND `+dataAccessSQL+`
...
func (s *Store) ListServersWithAccess(ctx context.Context, page, pageSize int, hubID int64, status, search string, access AccessFilter, sort Sort) ([]Server, Page, error) {
	...
	err := s.Pool.QueryRow(ctx, `SELECT count(*) FROM servers s LEFT JOIN managed_users u ON u.id=s.managed_user_id LEFT JOIN users account ON lower(account.username)=lower(s.username) WHERE ($1=0 OR s.hub_id=$1) AND ($2='' OR s.status=$2) AND ($3='' OR s.username ILIKE $4 ESCAPE '\') AND `+countAccessSQL, countArgs...).Scan(&total)
	if err != nil {
		return nil, Page{}, err
	}
