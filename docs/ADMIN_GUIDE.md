...
| `mail.username` · `mail.password` | 빈 값 | 인증 없는 릴레이가 흔하므로 **선택 사항**. 비밀번호는 암호화 저장되며 화면·API에는 "설정됨"만 보인다. 릴레이 주소를 바꾸면 다시 입력해야 한다 |
...
- 발송은 요청과 무관하게 배경에서 일어나므로 릴레이가 죽어 있어도 승인·Hub 동기화 같은 요청은
  평소처럼 끝납니다. 실패한 발송은 2초 뒤 한 번 더 시도하고 기록에 남깁니다.
