...
이 문서는 jupiq **v1.8.2**을 오프라인망에 설치하고 운영하는 사람을 위한 것입니다.
...
printf '%s  %s\n' "${JUPIQ_ARCHIVE_SHA256}" jupiq-v1.8.2.tar.gz | sha256sum -c -
gzip -t jupiq-v1.8.2.tar.gz
gzip -dc jupiq-v1.8.2.tar.gz | docker load
docker image inspect jupiq:v1.8.2 --format '{{index .Config.Labels "org.opencontainers.image.version"}}'
```
