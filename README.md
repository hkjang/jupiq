...
```text
Docker image   jupiq:v1.3.0
Release asset  jupiq-v1.3.0.tar.gz
```

```bash
# GitHub Release 본문의 64자리 SHA-256을 승인 기록과 대조
JUPIQ_ARCHIVE_SHA256='릴리스-본문의-SHA256'
printf '%s  %s\n' "${JUPIQ_ARCHIVE_SHA256}" jupiq-v1.3.0.tar.gz | sha256sum -c -
gzip -t jupiq-v1.3.0.tar.gz
gzip -dc jupiq-v1.3.0.tar.gz | docker load
