...
export function UsersPage() {
  const { user } = useAuth()
  return <ResourceListPage title="통합 사용자" description="모든 Hub의 사용자, 조직, 활동과 자원 현황을 조회합니다." endpoint="/users" emptyDescription="동기화된 사용자가 없습니다." columns={managedUserColumns(canOpenUserDetail(user?.global_permissions))} />
}
...
export function ServersPage() {
  ...
  return <ResourceListPage title="Notebook 서버" description="사용자 서버의 실행 상태와 자원을 확인하고 안전하게 제어합니다." endpoint="/servers" rowActions={actions} scopeAwareWrite emptyDescription="조회된 Notebook 서버가 없습니다." columns={[
...
export function GpusPage() {
  ...
  return <ResourceListPage title="GPU 모니터링" description="실행 서버·Pod·사용자별 GPU 할당과 수집 지표를 확인합니다." endpoint="/gpus" emptyDescription="수집된 GPU 메트릭이 없습니다." dataNote={<Alert type="info" showIcon message="GPU Util이 장시간 낮은 할당은 낭비 후보로 분류됩니다." description="DCGM Exporter 지표를 Pod 단위로 집계합니다. GPU 모니터링을 끄면 수집·API·화면 노출이 모두 중단됩니다." />} columns={[
...
export function ProjectsPage() {
  return <ResourceListPage title="프로젝트" description="구성원, 기간과 계획 자원 한도를 프로젝트 단위 카탈로그로 관리합니다." dataNote={<Alert type="info" showIcon message="v1.1은 프로젝트·Quota 등록과 조회를 제공합니다" description="JupyterHub 또는 Kubernetes에 Quota를 자동 집행하지는 않습니다." />} endpoint="/projects" writePermission="project:write" createLabel="프로젝트 등록" emptyDescription="등록된 프로젝트가 없습니다." fields={[
...
export function PoliciesPage() {
  return <ResourceListPage title="정책" description="망·그룹·사용자별 운영 정책을 중앙 카탈로그로 등록하고 비교합니다." dataNote={<Alert type="info" showIcon message="정책 카탈로그" description="v1.1은 정책 자동 배포·Idle 종료·Quota 집행을 수행하지 않습니다." />} endpoint="/policies" writePermission="policy:write" createLabel="정책 작성" emptyDescription="등록된 정책이 없습니다." fields={[
...
export function ProfilesPage() {
  return <ResourceListPage title="환경 프로필" description="CPU, RAM, GPU와 실행 이미지 조합을 표준 템플릿 카탈로그로 관리합니다." dataNote={<Alert type="info" showIcon message="프로필 카탈로그" description="Hub Spawner 자동 배포와 rollback은 후속 범위입니다." />} endpoint="/profiles" writePermission="profiles:write" createLabel="프로필 등록" emptyDescription="등록된 환경 프로필이 없습니다." fields={[
...
export function ImagesPage() {
  return <ResourceListPage title="Notebook 이미지" description="외부에서 검증한 실행 이미지의 상태와 수명주기를 카탈로그로 관리합니다." dataNote={<Alert type="info" showIcon message="이미지 검증 결과는 외부 도구에서 등록합니다" description="jupiq v1.1 자체는 Registry 배포, SBOM 생성이나 취약점 스캔을 수행하지 않습니다." />} endpoint="/images" writePermission="image:write" createLabel="이미지 등록" emptyDescription="등록된 Notebook 이미지가 없습니다." fields={[
...
export function IncidentsPage() {
  return <ResourceListPage title="인시던트" description="Hub, Pod, GPU와 인증 장애를 수동 등록해 영향과 복구 상태를 추적합니다." dataNote={<Alert type="info" showIcon message="수동 인시던트 기록" description="수집 장애의 자동 인시던트 생성은 후속 범위입니다." />} endpoint="/incidents" writePermission="incident:write" createLabel="인시던트 등록" emptyDescription="등록된 인시던트가 없습니다." fields={[
...
export function CostsPage() {
  ...
  return <ResourceListPage title="비용·용량" description="관리자가 등록한 부서·프로젝트·망별 비용 산정 결과를 조회합니다." dataNote={<Alert type="info" showIcon message="비용 결과 카탈로그" description="자동 단가 환산과 용량 포화 예측은 후속 범위입니다." />} endpoint="/costs" emptyDescription="등록된 비용 데이터가 없습니다." columns={columns} />
}

export function NotificationsPage() {
  return <ResourceListPage title="알림 센터" description="운영 알림 규칙과 수동 기록을 카탈로그로 관리합니다." dataNote={<Alert type="info" showIcon message="알림 카탈로그" description="Webhook은 관리자 연결 테스트만 제공하며 운영 이벤트 자동 발송은 후속 범위입니다." />} endpoint="/notifications" writePermission="notification:write" createLabel="알림 규칙 등록" emptyDescription="표시할 알림이 없습니다." fields={[
